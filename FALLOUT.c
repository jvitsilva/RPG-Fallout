// Autores: João Vitor, Carlos Henrique e João Renan
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <string.h>
...
//Distribuição de pontos nos atributos padrões:
				...
				printf("encaixar um dialogo estilo Mass Effect sem tornar o codigo 3 vezes mais complexo. De qualquer forma eh um milagre voce estar falando. Respondendo sua\n");Sleep(300);
				...
				system("pause");
				system("cls");
				printf("-%s e Richarlison partem em busca de uma das antigas faccoes da Comunidade, aproveitando a  escuridao noturna para se tornarem mais furtivos. Antes que os\n",personagem.nome);Sleep(300);
				...
				system("cls");
				Assembly();
				printf("%s : Entao... aqui estamos nos. A profecia dos antigos ancioes diz que o dia final do imperio comeca hoje, mas eles podem estar errados.Mas antes\n",inimigo.nome);Sleep(300);
				...
				printf("-Voce rapidamente puxa a sua arma e destrava.");Sleep(300);
                ...
                if(personagem.pv<=0 && inimigo.vida>0) {
					...
					printf("o irrefreavel exercito avanca por todo o territorio, voce nunca mais sera lembrado...");Sleep(300);
					printf("FIM\?");Sleep(300);
					printf("       \t\tAperte 1 para voltar para o menu\n");Sleep(300);
					printf("       \t\t->");Sleep(300);
					scanf("%i", &menu);
					system("cls");
...
void Assembly(){
	...
	personagem.nome[strlen(personagem.nome) - 1] = '\0';
	inimigo.dano=personagem.itens.dano+10;
	inimigo.defesa=personagem.itens.defesa+100;
	inimigo.vida=personagem.pv+100;
	inimigo.vidatotal=inimigo.vida;
	inimigo.defpadrao=inimigo.defesa;
	inimigo.ataquespecial=110;
	inimigo.carga=1;
...
void BOSS(){
	...
	system("cls");
	printf("|PERSONAGEM: %s\t\t\t|ADVERSARIO: %s\n",personagem.nome,inimigo.nome);
	printf("|ARMADURA: %i/%i\t\t\t|ARMADURA: %i/%i\n",personagem.itens.defesa,personagem.itens.defpadrao1,inimigo.defesa,inimigo.defpadrao);
	printf("|VIDA: %i/%i\t\t\t\t|VIDA: %i/%i\n",personagem.pv,personagem.pvtotal, inimigo.vida, inimigo.vidatotal);
	printf("|DANO: %i\t\t\t\t|DANO: %i\n", personagem.itens.dano,inimigo.dano);
	printf("|STIMPAK: %i/5\t\t\t\t|STIMPAK: %i/5\n\n",abs(stimpak1-5),abs(stimpak2-5));

//O PERSONAGEM SEMPRE COMECA PRIMEIRO
//IF CASO O PERSONAGEM AINDA TENHA ARMADURA 
...
if(stimpak1!=5){
	...
	printf("1-ATACAR\n");
	printf("2-DEFENDER\n");
	printf("3-USAR BFG9000\n");
	printf("4-USAR STIMPAK\n");
	printf("5-DESVIAR DO ATAQUE\n");
	printf("COMANDO:");
	scanf("%i",&comando);
...
switch(comando){
	...
	case 1:
	attack1=1;
//PRIMEIRO VAMOS CHECAR SE ELE AINDA TEM ARMADURA:
	if(inimigo.defesa>0){
	soma=inimigo.defesa-personagem.itens.dano;

    //SE EU ATACAR, A ARMADURA SERA DESTRUIDA?
    //SE SIM:	
	if(soma<=0){
	inimigo.defesa=0;
	printf("-A armadura de %s foi completamente destruida\n",inimigo.nome);
	printf("-A BFG9000 recebeu mais 1 carga para ser usada\n");
	personagem.itens.carga=personagem.itens.carga+1;
	attack1=0;	
	}
	
	//CASO CONTRARIO VAMOS DANIFICAR A ARMADURA
	else if(soma>0){
	inimigo.defesa=inimigo.defesa-personagem.itens.dano;
	printf("Voce causou um dano de %i na armadura de %s\n",personagem.itens.dano, inimigo.nome);	
	}	
...
//VAMOS NOS DEFENDER	
	...
	if(attack2==1){
	personagem.itens.defesa=personagem.itens.defesa+inimigo.dano;
	personagem.itens.defesa=personagem.itens.defesa-(inimigo.dano/10);	
	printf("-Voce se defendeu e o dano causado foi de apenas %i na armadura.\n", (inimigo.dano/10) );		
	}
	
	//SE A RESPOSTA FOR NÃO
	else if(attack2==0){
	printf("-Voce se defendeu de um possivel ataque,porem nenhum dano foi causado.\n");	
	}
	break;
	...
	case 3:
	//A BFG9000 AINDA TER CARGA?
	//SE A RESPOSTA FOR SIM:
	if(personagem.itens.carga!=0){
		
	//PRIMEIRO VAMOS CHECAR SE ELE AINDA TEM ARMADURA:
	if(inimigo.defesa>0){
	soma=inimigo.defesa-personagem.itens.BFG9000;

    //SE EU ATACAR, A ARMADURA SERA DESTRUIDA?
    //SE SIM:	
	if(soma<=0){
	inimigo.defesa=0;
	printf("-Voce empunha a BFG9000 e dispara, %s sofre um dano em sua armadura de %i pontos.\n",inimigo.nome, personagem.itens.BFG9000);
	printf("-A armadura de %s foi completamente destruida\n",inimigo.nome);	
	printf("-A BFG9000 recebeu mais 1 carga para ser usada\n");
	personagem.itens.carga=personagem.itens.carga+1;	
	}
	...
	inimigo.defesa=inimigo.defesa - personagem.itens.BFG9000;
	printf("-Voce empunha a BFG9000 e dispara, %s sofre um dano em sua armadura de %i pontos.\n",inimigo.nome, personagem.itens.BFG9000);	
	}	
    }
    ...
    inimigo.vida=inimigo.vida-personagem.itens.dano;
    printf("-Voce empunha a BFG9000 e dispara, %s sofre um dano em sua vida de %i pontos.\n",inimigo.nome, personagem.itens.BFG9000);
    }
    personagem.itens.carga=personagem.itens.carga-1;
    }
    
    
    //SE A BFG NÃO TIVER MAIS CARGA:
    else if(personagem.itens.carga==0){
    printf("-A BFG9000 nao possui mais municao.\n");
	}
	break;
...
//SE A RESPOSTA FOR SIM:
	...
	if(personagem.itens.defesa!=0){
	personagem.itens.defesa=personagem.itens.defesa+inimigo.dano;
	printf("-Voce se desvia do ataque inimigo, nenhum dano foi causado na sua armadura\n");
	}
	
	
	else if(personagem.itens.defesa==0 && personagem.pv==200){
	printf("-Voce ate tenta se desviar mas nada pode ser feito\n");	
	}
	
	else if(personagem.itens.defesa==0 && personagem.pv!=200){
	personagem.pv=personagem.pv+inimigo.dano;
	printf("-Voce se desvia do ataque inimigo, nenhum dano foi causado em voce\n");	
	} 	
	}
	
//CASO CONTRARIO:
	else if(attack2==0){
	printf("-Voce se desvia do possivel dano causado pelo inimigo, mas ele nao te atacou\n");	
...
//FIM DA ESTRUTURA CASO ELE TENHA ARMADURA





//INICIO DA ESTRUTURA CASO ELE NÃO TENHA ARMADURA
else if(personagem.itens.defesa<=0){
...
//FIM DA ESTRUTURA CASO ELE NÃO TENHA ARMADURA	
//FIM DE TODAS AS POSSIBILIDADES DO PERSONAGEM	
...
//PRIMEIRO VAMOS CHECAR SE ELE AINDA TEM ARMADURA:
	...
	if(soma<=0){
	personagem.itens.defesa=0;
	printf("-A sua armadura foi completamente destruida\n");
	printf("-%s recebeu mais 1 ataque especial para ser usado\n",inimigo.nome);
	inimigo.carga=inimigo.carga+1;	
	attack2=0;
	}
	...
	personagem.itens.defesa=personagem.itens.defesa-inimigo.dano;
	printf("-%s causou um dano de %i na sua armadura\n", inimigo.nome, inimigo.dano);	
	}	
		
	}
	
//CASO NAO TENHA:
    else if(personagem.itens.armadura<=0){
    personagem.pv=personagem.pv-inimigo.dano;
	printf("-Voce sofreu um dano de %i\n",inimigo.dano);	
    }
	break;
...
//VAMOS NOS DEFENDER
	...
	if(attack1==1){
	inimigo.defesa=inimigo.defesa+personagem.itens.dano;
	inimigo.defesa=inimigo.defesa-(personagem.itens.dano/10);	
	printf("-%s se defendeu e o dano causado foi de apenas %i na armadura.\n",inimigo.nome,(personagem.itens.dano/10) );		
	}
	
	//SE A RESPOSTA FOR NÃO
	else if(attack2==0){
	printf("-%s se defendeu de um possivel ataque, porem nada pode ser feito.\n",inimigo.nome);	
	}
	break;
	...
	case 3:
	//ATAQUE ESPECIAL AINDA TER CARGA?
	//SE A RESPOSTA FOR SIM:
	if(inimigo.carga!=0){
		
	//PRIMEIRO VAMOS CHECAR SE ELE AINDA TEM ARMADURA:
	if(personagem.itens.defesa>0){
	soma=personagem.itens.defesa-inimigo.ataquespecial;

    //SE EU ATACAR, A ARMADURA SERA DESTRUIDA?
    //SE SIM:	
	if(soma<=0){
	personagem.itens.defesa=0;
	printf("-%s dispara o raio da morte, voce sofre um dano em sua armadura de %i pontos.\n",inimigo.nome, inimigo.ataquespecial);
	printf("-A sua armadura foi completamente destruida\n");	
	printf("-Raio da morte recebeu +1 de carga\n");
	inimigo.carga=inimigo.carga+1;	
	}
	
	//CASO CONTRARIO VAMOS DANIFICAR A ARMADURA
	else if(soma>0){
	personagem.itens.defesa=personagem.itens.defesa-inimigo.ataquespecial;
	printf("-%s dispara o raio da morte, voce sofre um dano em sua armadura de %i pontos.\n",inimigo.nome, inimigo.ataquespecial);	
	}	
    }
    ...
    personagem.pv=personagem.pv-inimigo.ataquespecial;
    printf("-%s dispara o raio da morte, voce sofre um dano vital de %i pontos.\n",inimigo.nome, inimigo.ataquespecial);
    }
    inimigo.carga=inimigo.carga-1;
    }
    ...
    //SE A BFG NÃO TIVER MAIS CARGA:
    else if(inimigo.carga==0){
    printf("-Raio da morte nao possui mais municao.\n");
	}
	break;
	...
	soma=dado1l6+dado2l6+dado3l6;
	inimigo.vida=inimigo.vida+soma;
	printf("-Stimpak te forneceu  a %s, %i a mais de vida\n",inimigo.nome, soma);
	printf("-%s possui %i de vida\n",inimigo.nome,inimigo.vida);
	stimpak2=stimpak2+1;
	break;
...
//SE A RESPOSTA FOR SIM:
	...
	if(inimigo.defesa!=0){
	inimigo.defesa=inimigo.defesa+personagem.itens.dano;
	printf("-%s se desvia do ataque, nenhum dano foi causado na sua armadura\n",inimigo.nome);
	}
	
	
	else if(inimigo.defesa==0 && inimigo.vida==300){
	printf("-%s ate tenta se desviar mas nada pode ser feito\n",inimigo.nome);	
	}
	
	else if(inimigo.defesa==0 && inimigo.vida!=300){
	personagem.pv=personagem.pv+inimigo.dano;
	printf("-%s se desvia do seu ataque, nenhum dano foi causado\n",inimigo.nome);	
	} 	
	}
...
//SE A RESPOSTA FOR SIM:
	...
	if(inimigo.defesa!=0){
	inimigo.defesa=inimigo.defesa+personagem.itens.dano;
	printf("-%s se desvia do seu ataque, nenhum dano foi causado na sua armadura\n",inimigo.nome);
	}
...
//FIM DAS ESTRUTURAS DE DECISÃO CASO O BOSS AINDA TENHA ARMADURA


//INICIO DAS ESTRUTURAS DE DECISÃO CASO O BOSS NÃO TENHA ARMADURA
else if(inimigo.defesa<=0){
